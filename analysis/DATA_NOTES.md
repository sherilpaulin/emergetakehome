...
- **Assumption:** the official funnel (`analysis/funnel_analysis.py`) uses p90 recency cutoffs (7 / 55 / 78 days, signup-based) rather than the raw funnel, mean, or median. **Why:** recent signups haven't had time to finish (CLAUDE.md), and the raw funnel's CC/Permit rates were understated by counting too-recent signups as failures. p90 excludes only the ~10% slowest-but-still-legitimate finishers from each step's denominator; mean/median were considered and rejected because a median cutoff alone excludes roughly half of eventual finishers by definition, which defeats the purpose of the adjustment. See `ai_usage/STEERING_LOG.md` S-010 for the full discussion.
- **Assumption:** the passed/stopped/end status grouping used in `analysis/finish_vs_stop_comparison.py` (and any later analysis reusing it) is: **passed** = `status == permit_passed`; **stopped** = `status` in `not_started`, `in_progress`, `inactive`, `course_complete`, `permit_scheduled` (still short of a permit, not at a terminal negative outcome); **end** = `status` in `withdrawn`, `permit_failed` (excluded from the passed/stopped comparison as separate terminal endpoints). **Why:** `withdrawn` and `permit_failed` represent a definite negative resolution, not an ongoing "still could pass" state, so lumping them into "stopped" would conflate two different things. Confirmed explicitly by the user (`ai_usage/STEERING_LOG.md` S-014).
...
- **Question:** what do the `paid_social` ad campaigns actually say and target? `paid_social` converts signup-to-permit at 9.3% vs. 13.2-15.4% for every other referral source (I-021), is weakest at every funnel step, and shows much lower group chat participation (23.8% vs. 37-39.5% elsewhere) -- but does NOT show lower initial commitment (training-plan rate is actually the highest of any source, ruling that explanation out). Is there a lead-quality or expectation-setting issue in the ads themselves? **Until we know:** treat `paid_social`'s weakness as a likely acquisition/lead-quality issue rather than a training-content issue, and track it as its own segment rather than assuming a course fix would help it.
- **Question:** do `workforce_center` partner sites provide case management, job-placement incentives, or in-person support that specifically pushes clients from Course Complete to actually scheduling and passing the permit exam? `workforce_center` has the best Permit/CC rate of any referral source (54.7%) despite middling earlier steps, and this isn't explained by more post-completion coaching (coach call counts are nearly identical across all sources). **Until we know:** treat `workforce_center`'s Permit/CC strength as likely due to a partner-side structural factor not captured in this data; if confirmed, consider whether it's replicable for other referral sources.
