...
8. **[Pattern -- self-reported, individual-level only]** DMV transport access lines up with never scheduling the exam -- 170 of 257 (66%) of unpassed completers who never booked reported no/unsure transport or never filled out a plan -- but this does NOT explain city-level gaps, since self-reported transport access is nearly identical across Boston, NYC, and Sacramento (58-60% "yes" everywhere). (I-010/I-026)
9. **[Strong evidence of the gap; mechanism inferred]** Device matters -- `laptop` converts end-to-end at 13.9% vs. `phone`'s 11.6% (phone is 65.7% of all students, so this is real volume), and `shared_computer` users activate best of any device (76.7% reach First Video) but finish the course worst (27.5%), consistent with losing repeated access rather than an activation problem. (I-025)
10. **[Pattern -- self-selection likely]** Study hall attendance, not group chat membership, is the real support signal -- attendees pass at 27.9% vs. 9.0% for neither, while group chat alone barely moves the needle (10.0%) -- but attending a live weekly session repeatedly is itself a sign of an already-motivated student, the same caveat as point 5's plan adherence. (I-029)
...
Of the 216 students who stalled right after lesson 3 (Basic Control & Shifting, 44 min), quiz scores (79.1 vs 79.8 avg), minutes watched (50.4 vs 49.4 avg), and rewatch ratio (1.15x vs 1.12x) all match students who continued past it -- the content isn't the barrier. Demographics (city, age band, device, referral source) also match the overall population within 1-2 points. The one real gap: only 22.7% of stalled-at-3 students joined the group chat, vs. 34.2% overall (11.5 points). **Correlational flag (CLAUDE.md rule 7):** group chat is self-selected; this doesn't show joining would have kept them going. Median time since their last lesson is 118.9 days -- long dormant, not a short pause. **Update: I-029 shows group chat alone barely predicts outcomes -- study hall attendance (gated behind group chat membership) is the real signal; read "group chat gap" here as a proxy for study hall access.** **Source:** `analysis/funnel_dropoff.py`.
...
## I-022: Boston's weak performance is mostly NOT explained by its referral-source mix

Boston skews toward `paid_social` (34.4% vs. 27.9% overall, already the weakest channel per I-021) and away from `friend_family` (12.8% vs. 16.1%). But composition only explains part of the gap: weighting each referral source's *national* rate by Boston's own mix predicts a 12.2% end-to-end Permit rate for Boston -- Boston's actual rate is 7.6% (12 of 157 eligible), a 4.6-point gap composition doesn't explain. 4 of Boston's 5 referral sources underperform their own national rate, some sharply (`friend_family` 0.0% vs. 13.2% national, `reentry_org` 5.4% vs. 13.4%, `paid_social` 5.8% vs. 9.3%) -- only `workforce_center` does better in Boston (23.8% vs. 15.4%, but n=21/5 passes). This points to something Boston-specific cutting across channels, not just a worse lead mix. **Caveat:** per-source-within-Boston samples are tiny (20-52 eligible, single-digit passes) -- real signal, not statistically solid at this n. **Source:** `analysis/funnel_analysis.py`.

## I-023: Boston isn't a content problem -- it looks like a coaching-capacity gap plus a language-access gap unique to that market

Boston's stalled students stop at the same lessons as everyone else (72.2% General Knowledge, 20.8% Orientation vs. 69.1%/23.0% overall), and quiz score/rewatch ratio match the overall population exactly -- content and comprehension aren't the issue (consistent with I-011/I-015 at the city level). Group chat participation is also similar to overall (32.8% vs. 34.2%). But coach calls show a real gap: Boston's median is 0, vs. 1 for the overall population (means: 0.74 vs. 0.84) -- Boston is the smallest market (25 seats vs. 200/75), consistent with a coaching-capacity/staffing gap rather than a behavioral one. Separately: all 25 Haitian Creole (`ht`) speakers in the entire dataset are in Boston (10% of Boston's population, 0% elsewhere), and their end-to-end Permit rate is 7.1% (1 of 14 eligible) vs. 12.3% (English) and 13.5% (Spanish) nationally -- ties directly to CLAUDE.md's rule to write to students in their `preferred_language` or mark `NEEDS TRANSLATION`. **Caveat:** n=14 eligible, 1 pass -- not statistically solid alone, but concrete and checkable. **Source:** `analysis/boston_dive.py`.

## I-024: NYC and Sacramento track each other closely -- Boston is the outlier, not just "the small city"

Running the same lesson/support/demographic checks (I-023) for all three cities: stall points and lesson performance are nearly identical everywhere (quiz score 79.4/79.4/79.8, rewatch ratio ~1.12-1.13 for Boston/NYC/Sacramento). Support signals diverge specifically for Boston: coach calls mean/median is 0.74/0 for Boston vs. 0.84/1 (NYC) and 0.87/1 (Sacramento) -- **Boston is the only city where the median student has zero coach calls**; study hall is also lowest in Boston (0.34 vs. 0.52/0.44). Group chat is similar across all three (32-35%), so this is a coaching-intensity gap specifically, not a general engagement gap. Demographically, NYC and Sacramento have zero Haitian Creole speakers between them -- confirms I-023's finding that Boston's entire `ht`-speaking population (10% of its students) is unique to that market. **Source:** `analysis/boston_dive.py`.

## I-025: Age doesn't predict outcomes; device does, and shared_computer users show a distinct pattern

End-to-end Permit rate by `age_band` is flat: 12.5% (18-24), 13.1% (25-34), 11.9% (35-44), 12.1% (45-54), 12.2% (55+) -- all within 1.2 points, not a differentiator. By `primary_device`: laptop 13.9%, phone 11.6%, tablet 18.4%, shared_computer 10.6% (tablet/shared_computer n=104-114, small). `phone` (65.7% of all students) converting below `laptop` is real volume, not noise. The standout is `shared_computer`: highest First Video rate of any device (76.7%) but the worst Course-Complete rate (27.5%, ~14 points below every other device) -- consistent with a "gets one session, can't get repeated access" pattern rather than an activation problem. **Source:** `analysis/funnel_analysis.py`.

## I-026: `plan_has_transport_to_dmv` is nearly identical across all 3 cities -- rules out transport as Boston's specific problem

Boston 58.0% yes / 22.2% unsure / 19.8% no; NYC 60.0% / 20.5% / 19.6%; Sacramento 57.9% / 22.2% / 19.9% -- essentially the same distribution everywhere. This closes off one hypothesis from I-023/I-024: Boston's underperformance is not explained by worse self-reported DMV transport access. The Boston story remains about coaching capacity (I-023) and its unique Haitian Creole-speaking population (I-023/I-024), not transport. **Source:** `analysis/funnel_analysis.py`.

## I-027: Meeting your own week-1 pace target is the strongest single signal found so far

Students with a plan who hit or exceeded their own chosen `plan_lessons_per_week` target in their first 7 days (`lessons_first_7_days >= plan_lessons_per_week`) pass at 35.9% (42 of 117 eligible) -- vs. 13.4% for plan-setters who fell short of their own target (142 of 1,057), and 7.2% for students with no plan at all (51 of 709). A ~5x gap vs. no plan and ~3x vs. missing your own target -- the biggest gap found in this analysis, bigger than any support signal. Correlational: early follow-through is itself a marker of an organized, motivated student, not proof that hitting a pace target causes success. **Source:** `analysis/behavior_analysis.py`.

## I-028: No lesson in the course is quietly harder than any other -- confirms I-011 applies everywhere, not just lesson 3

Across all 21 lessons, avg quiz score ranges 79.05-79.79 (a 0.74-point spread) and rewatch ratio ranges 1.11-1.14 -- completely flat. Content difficulty is not a factor anywhere in the curriculum, not just at the lesson-3 stall point already checked. **Source:** `analysis/behavior_analysis.py`.
